...
<head>
	...
	<script type="text/javascript" src="${pageContext.request.contextPath}/static/jquery-easyui-1.3.3/datagrid-scrollview.js"></script>
	<script type="text/javascript">
        ...
        function deleteStudent(){
            ...
            $.messager.confirm("系统提示","您确认要删掉这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
                if(r){
                    $.post("deleteStudent",{delIds:ids},function(data){
                        if(data["deletesuccess"]){
                            $.messager.alert("系统提示","您已成功删除<font color=red>"+data["deletecount"]+"</font>条数据！");
                            $("#dg").datagrid("reload");
                        }else{
...
//            var rows=$('#dg').datagrid('getPager').data("pagination").options.pageSize;
            var sex=$('#s_sex').combobox("getValue");
            var gradeId=$('#s_gradeId').combobox("getValue");
//            "page="+page+"&rows="+rows+
            $.ajax({
                url:'${pageContext.request.contextPath}/management/searchStudent',
				type:'post',
				data:"&sex="+sex+"&gradeId="+gradeId,
				success:function (data) {
                    if (data["error"]==0) {
                        $.messager.alert("系统提示","没有符合搜索条件的学生");
                    } else {
        ...
        function openStudentAddDialog(){
            ...
            resetValue();
            controllerurl="${pageContext.request.contextPath}/management/addStudent";
        }
...
//						alert("添加失败");
                        return;
                    }else if(json['addsuccess']){
                        $.messager.alert("系统提示","保存成功");
//						alert("添加成功");;
                        $("#dlg").dialog("close");
                        $("#dg").datagrid("reload");
                    }else if(json['updatesuccess'])
                    {
                        $.messager.alert("系统提示","修改成功");
                        $("#dlg").dialog("close");
                        $("#dg").datagrid("reload");
					}else if(json['updatefail'])
					{
        ...
        function openStudentModifyDialog(){
            ...
            $("#fm").form("load",row);
            controllerurl="${pageContext.request.contextPath}/management/updateStudent";
        }
...
<table id="dg" title="学生信息" class="easyui-datagrid"
	   url="${pageContext.request.contextPath}/management/studentList"
...
<div id="tb">
	...
	<div id="search_tb">
		...
		&nbsp;所属班级：&nbsp;<input class="easyui-combobox" id="s_gradeId" name="s_gradeId" size="10" data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'gradeName',url:'${pageContext.request.contextPath}/management/gradeComboList'"/>
...
<div id="dlg" class="easyui-dialog" style="width: 570px;height: 350px;padding: 10px 20px"
	...
	<form id="fm" method="post">
		<table cellspacing="5px;">
			...
			<tr>
				<td>班级名称：</td>
				<td><input class="easyui-combobox" id="gradeId" name="gradeId"  data-options="panelHeight:'auto',editable:true,valueField:'id',textField:'gradeName',url:'${pageContext.request.contextPath}/management/gradeComboList'"/></td>
				<td></td>
				<td>Email：</td>
			...
			<tr>
			<td><input type="number" name="stuId" id="stuId"  required="false" hidden="true" defaultValue=""/></td>
			</tr>
		</table>
	</form>
</div>
